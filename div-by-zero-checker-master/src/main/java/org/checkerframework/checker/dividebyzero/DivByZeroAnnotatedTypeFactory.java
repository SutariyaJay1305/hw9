...
public class DivByZeroAnnotatedTypeFactory extends BaseAnnotatedTypeFactory {
     ...
     */
     ...
     final AnnotationMirror POSITIVE = AnnotationBuilder.fromClass(elements, Positive.class);
     final AnnotationMirror NEGATIVE = AnnotationBuilder.fromClass(elements, Negative.class);
     final AnnotationMirror ZERO = AnnotationBuilder.fromClass(elements, Zero.class);
    final AnnotationMirror TOP = AnnotationBuilder.fromClass(elements, Top.class);
    AnnotatedTypeMirror type = (AnnotatedTypeMirror) POSITIVE;
    ...
    private Class<? extends Annotation> defaultAnnotation(LiteralTree literal) {
        ...
        case INT_LITERAL:
            ...
            // TODO
            if (intValue == 0) {
                type.replaceAnnotation(ZERO);
            } else if (intValue > 0) {
                type.replaceAnnotation(POSITIVE);
            } else {
                type.replaceAnnotation(NEGATIVE);
            }
            break;
        case LONG_LITERAL:
